...
public:
    vector<vector<int>> flipAndInvertImage(vector<vector<int>>& A) {
        if(A.size()){
            ...
            vector<vector<int>> Answer(A.size(),B);
            for(int j=0;j<A.size();j++){
                for(int i=0;i<columns;i++){
                    Answer[j][i]=1^A[j][columns-i-1];
                }
            }
            return Answer;
        }
...
template<int row, int columns, int... I>
auto Invert(Matrix<row,columns,I...>){
    return Matrix<row,columns,(1-I)...>();
}

template <int index = 0, int... I>
auto flipN(Matrix<I...> _)
{
    if constexpr (index >= _.r * _.c)
        return int_sequence<>();
    else
    {
        constexpr int number=getvalue<(1+index/_.c)*_.c-1-index%_.c + 2>(int_sequence<I...>());
        return cat<number>(flipN<index+1>(_));
    }
}
...
template<int... I>
auto flipAndInvertImage(Matrix<I...> _){
    return Invert(make_matrix<_.r,_.c>(flipN(_)));
}
...
int main(){
    ...
    auto result=s.flipAndInvertImage(test);
    for(auto i:result){
        for (auto j:i)
            cout<<j<<" ";
        cout<<endl;
    ...
    cout<<endl;
    auto template_result=flipAndInvertImage(template_test);
